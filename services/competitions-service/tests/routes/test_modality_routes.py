...
async def test_create_modality(client: AsyncClient):
    ...
    response = await client.post("/api/modalities/", json=payload)
...
async def test_list_modalities(client: AsyncClient):
    ...
    await client.post("/api/modalities/", json={"name": "Futsal", "organization_slug": "IFCE"})
    await client.post("/api/modalities/", json={"name": "Basquete", "organization_slug": "IFCE"})

    response = await client.get("/api/modalities/")
