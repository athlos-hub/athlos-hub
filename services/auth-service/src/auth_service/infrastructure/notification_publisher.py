...
async def send_internal_notification(
    *,
    ...
    try:
        ...
        logger.info("Notificação %s enviada (HTTP) para %s", notification_type, user_id)
    except httpx.HTTPStatusError as e:
        logger.error(
            "Erro HTTP ao enviar notificação %s: %s", notification_type, e.response.text
        )
    except Exception as e:
        logger.error("Erro ao enviar notificação %s: %s", notification_type, e)
