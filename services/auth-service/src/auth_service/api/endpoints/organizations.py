...
from auth_service.api.deps import OrgRole, get_user_org_role
...
async def create_organization(
    ...
    exists_org = await session.scalar(
        select(Organization.id).where(Organization.slug == generated_slug)
    )

    if exists_org:
        raise HTTPException(
            status_code=409, detail="Já existe uma organização com esse nome."
        )
    ...
    owner_member = OrganizationMember(
        organization_id=new_org.id, user_id=current_user.id, status=MemberStatus.ACTIVE
    )
...
async def get_organization_by_id(
    ...
    if user:
        ...
        if org_role == OrgRole.NONE and org.privacy == OrganizationPrivacy.PRIVATE:
            logger.warning(
                f"Acesso negado à organização privada {org_slug} para usuário {user.id}"
            )
            raise HTTPException(
                status_code=403, detail="Acesso negado à organização privada."
            )

        if (
            org_role in {OrgRole.ORGANIZER, OrgRole.MEMBER}
            or org_role == OrgRole.NONE
            and org.privacy == OrganizationPrivacy.PUBLIC
        ):
            return OrganizationGetPublic.model_validate(org)

        raise HTTPException(
            status_code=403,
            detail="Você não tem a função necessária para visualizar esta organização.",
        )
    else:
...
async def update_organization(
    ...
    if org.owner_id != user.id:
        logger.warning(
            f"Tentativa de atualização não autorizada da organização {org_slug} por usuário {user.id}"
        )
        raise HTTPException(
            status_code=403,
            detail="Apenas o proprietário da organização pode atualizá-la.",
        )
    ...
    try:
        ...
        logger.warning(
            f"Conflito ao atualizar organização {org_slug}: dados duplicados"
        )
        raise OrganizationAlreadyExists(org_slug)
...
async def update_organization_join_policy(
    ...
    if org.owner_id != user.id:
        logger.warning(
            f"Tentativa de atualização da política de adesão não autorizada da organização {org_slug} por usuário {user.id}"
        )
        raise HTTPException(
            status_code=403,
            detail="Apenas o proprietário da organização pode atualizar a política de adesão.",
        )
    ...
    logger.info(
        f"Política de adesão da organização {org_slug} atualizada para {request.join_policy.value} por usuário {user.id}"
    )
...
async def request_to_join_organization(
    ...
    new_membership = OrganizationMember(
        organization_id=org.id, user_id=user.id, status=MemberStatus.PENDING
    )
...
async def cancel_join_request(
    ...
    logger.info(
        f"Usuário {user.id} cancelou sua solicitação de entrada na organização {org_slug}"
    )
    return
...
async def invite_user_to_organization(
    ...
    if not member_is_active:
        raise HTTPException(
            status_code=403, detail="O usuário convidado deve ter uma conta ativa."
        )
    ...
    new_membership = OrganizationMember(
        organization_id=org.id, user_id=user_id, status=MemberStatus.INVITED
    )
    session.add(new_membership)
    await session.commit()

    logger.info(
        f"Usuário {user.id} convidou o usuário {user_id} para a organização {org_slug}"
    )
...
async def cancel_invite(
    ...
    logger.info(
        f"Admin {user.id} cancelou o convite de {user_id} para a organização {org_slug}"
    )
    return
...
async def join_organization_via_link(
    ...
    new_membership = OrganizationMember(
        organization_id=org.id, user_id=user.id, status=MemberStatus.ACTIVE
    )
    session.add(new_membership)
    await session.commit()

    logger.info(
        f"Usuário {user.id} entrou na organização {org_slug} via link de convite"
    )
...
async def approve_join_request(
    ...
    logger.info(
        f"Admin {user.id} aprovou solicitação {membership_id} do usuário {membership.user_id} na organização {org_slug}"
    )
...
async def reject_join_request(
    ...
    logger.info(
        f"Admin {user.id} rejeitou solicitação {membership_id} do usuário {membership.user_id} na organização {org_slug}"
    )
...
async def list_pending_join_requests(
    ...
    logger.info(
        f"Usuário {user.id} listou {len(requests_list)} solicitações pendentes da organização {org_slug}"
    )
...
async def list_sent_invites(
    ...
    logger.info(
        f"Usuário {user.id} listou {len(invites_list)} convites enviados da organização {org_slug}"
    )
...
async def list_organization_members(
    ...
    logger.info(
        f"Usuário {user.id} listou {len(members_list)} membros da organização {org_slug}"
    )
...
async def list_organization_organizers(
    ...
    logger.info(
        f"Usuário {user.id} listou {len(organizers_list)} organizadores da organização {org_slug}"
    )
...
async def remove_member_from_organization(
    ...
    if not is_admin:
        logger.warning(
            f"Tentativa não autorizada de remoção do membro {user_id} da organização {org_slug} pelo usuário {user.id}"
        )
        raise HTTPException(
            status_code=403,
    ...
    if not membership:
        raise HTTPException(
            status_code=404, detail="Membro não encontrado na organização."
        )
    ...
    logger.info(
        f"Usuário {user.id} removeu o membro {user_id} da organização {org_slug}"
    )
    return
...
async def add_organizer_to_organization(
    ...
    if org.owner_id != user.id:
        logger.warning(
            f"Tentativa não autorizada de adicionar organizador {user_id} à organização {org_slug} pelo usuário {user.id}"
        )
        raise HTTPException(
            status_code=403,
    ...
    if existing_organizer:
        raise HTTPException(
            status_code=409, detail="O usuário já é um organizador desta organização."
        )
    ...
    logger.info(
        f"Usuário {user.id} promoveu {user_id} a organizador da organização {org_slug}"
    )
...
async def remove_organizer_from_organization(
    ...
    if org.owner_id != user.id:
        logger.warning(
            f"Tentativa não autorizada de remover organizador {user_id} da organização {org_slug} pelo usuário {user.id}"
        )
        raise HTTPException(
            status_code=403, detail="Apenas o proprietário pode remover organizadores."
        )

    if user_id == org.owner_id:
        raise HTTPException(
            status_code=400, detail="O proprietário não pode ser organizador."
        )
    ...
    if not existing_organizer:
        raise HTTPException(
            status_code=404, detail="O usuário não é um organizador desta organização."
        )
    ...
    logger.info(
        f"Usuário {user.id} removeu o cargo de organizador de {user_id} na organização {org_slug}"
    )
    return
...
async def transfer_organization_ownership(
    ...
    if request.new_owner_id == user.id:
        raise HTTPException(
            status_code=400, detail="Você já é o proprietário desta organização."
        )
    ...
    new_owner = await session.scalar(
        select(User).where(User.id == request.new_owner_id, User.enabled == True)
    )

    if not new_owner:
        raise HTTPException(
            status_code=404, detail="Usuário não encontrado ou conta inativa."
        )
    ...
    logger.info(
        f"Propriedade da organização {org_slug} transferida de {old_owner_id} para {request.new_owner_id}"
    )
