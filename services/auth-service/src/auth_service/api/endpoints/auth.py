...
from auth_service.domain.services.auth_service import AuthService, keycloak_openid
...
async def get_authenticated_user_info(
    db_user: User = Depends(AuthService.get_current_db_user),
):
    try:
        return db_user
...
async def keycloak_callback(payload: dict = Body(...)):
    ...
    try:
        ...
        if not access_token:
            logger.error(
                f"Resposta inválida do Keycloak ao trocar code: {token_response}"
            )
            raise HTTPException(
        ...
        db_user = await AuthService.get_or_create_user_from_keycloak_token(
            token_payload
        )

        try:
            await run_in_threadpool(
                AuthService.add_role_to_user, db_user.keycloak_id, "player"
            )
...
async def logout(request: LogoutRequest = Body(...)):
    try:
        await run_in_threadpool(
            keycloak_openid.logout, refresh_token=request.refresh_token
        )
    ...
    try:
        ...
        if avatar:
            try:
                ...
                logger.warning(
                    f"Erro no upload do avatar para usuário {new_user_id}: {e.detail}"
                )

        try:
            await run_in_threadpool(AuthService.add_role_to_user, new_user_id, "player")
        except Exception as role_error:
            logger.warning(
        ...
        try:
            ...
            logger.error(
                f"Erro ao sincronizar usuário {username} no banco: {e}", exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def verify_email(token: str):
    try:
        ...
        logger.warning("Tentativa de verificação com token expirado")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def resend_verification_email(
    payload: ResendEmailRequest, background: BackgroundTasks
):
    try:
        user_id = None
        user_email = None
        ...
        async with db.session() as session:
            ...
            if not user:
                logger.warning(
                    f"Tentativa de reenvio de verificação para email não encontrado: {payload.email}"
                )
                raise HTTPException(
            ...
            if user.email_verified:
                logger.info(
                    f"Tentativa de reenvio de verificação para email já verificado: {payload.email}"
                )
