...
from auth_service.core.exceptions import OrganizationNotFoundError, UserNotFoundError
...
async def get_users_public(session: AsyncSession = Depends(get_session)):
...
async def update_user_info(
    ...
    try:
        ...
        if not db_user:
            raise HTTPException(
                status_code=404, detail="Usuário não encontrado na sessão do banco"
            )
        ...
        if avatar:
            try:
                ...
                logger.warning(
                    f"Erro no upload do avatar para usuário {user.id}: {e.detail}"
                )
                raise

        if updates_keycloak:
            await run_in_threadpool(
                keycloak_admin.update_user, user.keycloak_id, updates_keycloak
            )
            logger.info(
                f"Usuário {user.keycloak_id} atualizado no Keycloak: {list(updates_keycloak.keys())}"
            )

        if updates_db:
            stmt_update = (
                sa_update(User).where(User.id == db_user.id).values(**updates_db)
            )
            await session.execute(stmt_update)
            await session.commit()

            logger.info(
                f"Usuário {db_user.id} atualizado no banco de dados: {list(updates_db.keys())}"
            )
        ...
        if refreshed_user:
            ...
            logger.debug(
                f"Dados finais do usuário: first_name={refreshed_user.first_name!r}, last_name={refreshed_user.last_name!r}, username={refreshed_user.username!r}"
            )
...
async def leave_organization(
    ...
    if not membership:
        raise HTTPException(
            status_code=404, detail="Você não é um membro ativo desta organização."
        )
