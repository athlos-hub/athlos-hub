...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleResponseStatusException(
        ...
        boolean isModerationRejected = status.value() == 422;

        ErrorResponse errorResponse = ErrorResponse.of(
                status.value(),
                status.getReasonPhrase(),
                reason,
                isModerationRejected ? "MODERATION_REJECTED" : "REQUEST_FAILED",
                request.getRequestURI()
        );
        ...
        log.warn("Request failed with status {}: {} - Path: {}",
                status.value(), reason, request.getRequestURI());
