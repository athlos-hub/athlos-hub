...
@router.get("/profile-by-username/{username}")
async def profile_by_username(
    username: str,
    session: AsyncSession = Depends(get_session),
):
    """Obtém o perfil do atleta pelo username."""
    p = await get_athlete_by_username(session, username)
    return api_success(athlete_profile_to_camel(p))
...
async def org_profile_get(slug: str, session: AsyncSession = Depends(get_session)):
    p = await require_org_social_visible(session, slug)
