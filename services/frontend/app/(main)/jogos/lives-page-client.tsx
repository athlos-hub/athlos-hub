...
function parseStatusFromSearchParams(sp: URLSearchParams): LiveStatus | "all" {
  ...
  if (Object.values(LiveStatus).includes(raw as LiveStatus)) return raw as LiveStatus;
  return "all";
}
...
export default function LivesPageClient() {
  ...
  const statusFilter = parseStatusFromSearchParams(searchParams);

  const jogosPathWithQuery = useMemo(() => {
  ...
  useEffect(() => {
    async function loadLivesWithMatchData() {
      ...
      try {
        ...
        const livesData = await listLives(listParams);
  ...
  const handleAddToCalendar = async (liveId: string) => {
    ...
    try {
      ...
      if (!isGoogleCalendarAuthorized) {
        const oauthUrl = await getGoogleCalendarOAuthUrl(jogosPathWithQuery);
  ...
  return (
    <>
      <div className="min-h-screen space-y-6">
        <PageHeader
          title="Jogos"
          subtitle="Assista as transmissões ao vivo da sua equipe preferida"
        />

        {sessionStatus === "unauthenticated" && (
          <LoginCtaBanner
            description="A lista de jogos e transmissões é pública. Entre na sua conta para sincronizar partidas com o Google Calendar e usar recursos extras."
            callbackPath={jogosPathWithQuery}
          />
        )}
        ...
        <FilterPanel icon={<Filter className="w-5 h-5 text-gray-600" />}>
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={() => applyStatusFilter(LiveStatus.SCHEDULED)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              ...
              <button
                onClick={() => applyStatusFilter(LiveStatus.LIVE)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              ...
              <button
                onClick={() => applyStatusFilter(LiveStatus.FINISHED)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
